...
def restrictlist(minval, sequence, maxval):
...
class Game(object):
	...
	def __init__(self):
		...
		self.screen = pygame.display.set_mode((round(pygame.display.get_surface().get_size()[0]*0.80)-self.widthreduc,\
			round(pygame.display.get_surface().get_size()[1]*0.9)-self.heightreduc), pygame.RESIZABLE)
	...
	def toggleFullscreen(self, full):
		if full:
			self.smallwidth = self.screenwidth
			self.smallheight = self.screenheight
			self.screen = pygame.display.set_mode((1920, 1080), pygame.NOFRAME)
		else:
			self.screen = pygame.display.set_mode((self.smallwidth,  self.smallheight), pygame.RESIZABLE)
		self.setScreenSize()
...
class World(object):
	...
	def __init__(self, name, width, height, color, exteriorcolor, tilesize, walled, elliptical, miniature, wallwidth=0, wallcolor=0, borderdamage=0, borderspeed=1, slip=defaultslip, shown=True, locked=False, gravity=0):
	...
	def draw(self):
		...
		exteriorcolor = restrictlist(0, self.exteriorcolor, 255)
		
		if (player.x < (self.width/2)-(game.screenwidth/2) and player.x > -(self.width/2)+(game.screenwidth/2)) and \
		 (player.y < (self.height/2)-(game.screenheight/2) and player.y > -(self.height/2)+(game.screenheight/2)):
			drawexterior = False
		else:
			drawexterior = True
		...
		self.screenX = round(player.x-(self.width/2) + game.screenwidth/2)
		self.screenY = round(player.y-(self.height/2) + game.screenheight/2)
		...
		if self.wallwidth and self.wallcolor: 
			...
			wallcolor = restrictlist(0, self.wallcolor, 255)
			#shape(game.screen, wallcolor, (self.screenX-self.wallwidth, self.screenY-self.wallwidth, self.width+self.wallwidth*2, self.height+self.wallwidth*2)
			rects = []
			...
			rects.append((self.screenX-wallwidth, self.screenY+height, width+wallwidth*2, wallwidth))
		...
		if drawexterior:
			shape(game.screen, color, (self.screenX, self.screenY, self.width, self.height))
...
# world definitions
town = World('Town', 8000, 8000, (86, 200, 76), (220, 200, 140), 100, False, False, False)
house_interior = World('House Interior', 1050, 850, (230, 210, 140), town.color,  75, True, False, True, wallwidth=25, wallcolor=(100, 80, 25))
shop_interior = World('Shop Interior', 1200, 750, (230, 210, 140), town.color, 75, True, False, True, wallwidth=50, wallcolor=(250, 100, 100))
#school_interior = World('Shop Interior', 1200, 750, (230, 210, 140), town.color, 75, True, False, True, wallwidth=50, wallcolor=(250, 100, 100))
city = World('City', 6000, 6000, (170, 170, 170), (110, 220, 130), 100, False, False, False)
apartment_interior = World('Apartment Interior', 1125, 1575, (220, 200, 140), city.color, 75, True, False, True, wallwidth=75, wallcolor=(200, 140, 50))
...
heck = World('Heck', 4500, 4500, (100, 35, 30), (255, 150, 0), 100, False, False, False, False, borderdamage=6, borderspeed=0.5) #no swearing on my christian minecraft server
...
daddyland = World('Daddyland', 6900, 4200, (255, 195, 240), (255, 50, 170), 75, True, False, False, True, borderspeed=0.1)
lab_interior = World('Lab Interior', 1500, 1500, (120, 120, 120), city.color, 50, True, False, True, wallwidth=100, wallcolor=(250, 250, 250))
desktop = World('Desktop', 1500, 1500, (0, 0, 0), (100, 100, 100), 75, True, False, True, wallwidth=150, wallcolor=(50, 50, 50), shown=False, locked=True)
...
class Character(object):
	...
	def draw(self):
		...
		self.color = restrictlist(0, self.color, 255)
				...
				if game.BUTTON1:
					...
					if 'Range' in self.inventory[self.selected] and self.get_selected_val('Range') > 0:
						...
						if self.selected == 'Bow': 
							...
							newprojectile = Projectile(self.x, self.y, angle, 85, 30, size, size, (125, 100, 50), pygame.draw.ellipse, False, self.setDamage())
					self.checkDeplete()
				if game.BUTTON2:
	...
	def drawItems(self):
		...
		if self.get_selected_val('Type') == 'Weapon' and game.COOLDOWN < 5:
...
class Projectile(object):
	...
	def loop(self):
		if self.launched:
			...
			self.rect = pygame.Rect(int(self.screenX), int(self.screenY), self.width, self.height)	
			#self.onscreen = True#self.rect.colliderect(game.screen.get_rect())
...
class Component(object):
	...
	def __init__(self, name, x ,y, width, height, color, world, showname, elliptical=False, health=maxhealth, textsize=50, damage=0, playerspeed=1, slip=defaultslip, imagename='', movespeed=0, pos2=(), textcolor=(255, 255, 255), overrideshow=False):
...
h_road = Component('Horizontal Road', 0, 0, town.width*1.25, 250, (50, 50, 50), town, False, False)
v_road = Component('Vertical Road', 0, 0, 250, town.height*1.25, (50, 50, 50), town, False, False)
...
office = Component('Office',  -490, -400, 300, 300, (160, 180, 180), town, True, False)
...
# items
cash = Component('$', town.randX(), town.randY(), 58, 30, (100, 150, 100), town, True, False, textsize=40, textcolor=(175, 250, 175), overrideshow=True)
cash2 = Component('$', town.randX(), town.randY(), 58, 30, (100, 150, 100), town, True, False, textsize=40, textcolor=(175, 250, 175), overrideshow=True)
...
# City components
# streets
h_street = Component('Horizontal Street', 0, 0, city.width, 300, (50, 50, 50), city, False, False)
v_street = Component('Vertical Street', 0, 0, 300, city.height, (50, 50, 50), city, False, False)
...
laboratory = Component('Laboratory', 1000, -425, 300, 350, (250, 250, 250), city, True, False, textcolor=(0, 0, 0))

# city items
credit_card = Component('Credit Card', city.randX(), city.randY(), 64, 36, (220, 200, 150), city, True, False, textsize=17)
...
dl_sign = Component('Daddyland Sign', 0, daddyland.height/2-500, 300, 200, (255, 255, 255), daddyland, False, False, imagename='daddyland.jpeg')
...
class Block(object):
	...
	def draw(self):
		...
		if self.world == player.world and self.deleted == False:
			...
			#pygame.draw.rect(game.screen, (0, 0, 0),(player.screenX-player.radius+4, player.screenY-player.radius+4, player.radius*2-8, player.radius*2-8))
		
	
	def loop(self):
		self.screenX = player.x+(-self.x-(self.width/2))+(game.screenwidth/2)
		self.screenY = player.y-(self.y+(self.height/2))+(game.screenheight/2)
		self.rect = pygame.Rect(int(self.screenX), int(self.screenY-self.tallness), self.width, self.height+int(self.tallness))
		self.onscreen = self.rect.colliderect(game.screen.get_rect())
	...
	def collision(self):
		...
		#if player.world ==  self.world and self.solid and Block.is3D and not player.blockoverlapping: # (Currently disabled) collision detector for solid components/blocks
