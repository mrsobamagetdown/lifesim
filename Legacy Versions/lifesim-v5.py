...
class Game:
	...
	def draw(self):
		...
		#if self.fast
		#	if self.updateright:
		#		pygame.display.update((0, 0, self.get_screenwidth()/2, self.get_screenheight()))
		#	else:
		#		pygame.display.update(pygame.display.update((self.get_screenwidth()/2, 0, self.get_screenwidth()/2, self.get_screenheight())))
		#	self.updateright = not self.updateright
...
class Character:
	...
	def __init__(self, name):
		...
		for image in self.images:
			self.images[image] = pygame.image.load(image + '.png').convert_alpha()
	...
	def manageSpeed(self):
		...
		speedmultiplier *= 1.35#(((self.world.width+self.world.height)/2)/10000)+1
	...
	def checkPlaceable(self):
		...
		for item in game.blocks:
			if item.x == self.blockX and item.y == self.blockY and item.world == self.world and not item.deleted:
				placeable = False # Prevents the block from overwriting antoher block.
		return placeable
	...
	def draw(self):
		if self.shown and self.onscreen:
			if self.image:
				...
				color = restrictlist(self.color, 0, 255)
...
rock = Component('$', -1100, 850, 100, 150, (125, 125, 125), town, False)
...
dl_sign = Component('Daddyland Sign', 0, daddyland.height/2-500, 300, 200, (255, 255, 255), daddyland, imagename='daddyland.jpeg', tp=club)
